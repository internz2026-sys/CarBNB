...
| Wrong tab on login + wrong card on signup mirror email/password rules | **OAuth callback enforces the same role-mismatch checks `loginAction` and `signupAction` already enforce.** Customer email clicking Continue with Google on the Host login tab → signed out + bounced to `/login?error=customer_exists`. Existing host email clicking Sign up with Google on the Customer card → bounced to `/signup?error=host_exists`. Same-role collision on signup → `/signup?error=account_exists`. Admin (User row) bypasses the check, same as email/password | Tier 18 | Behavior parity between the two providers — users don't have to learn "OAuth is different" rules. The error wording matches `loginAction` so the messages reuse familiar copy. |
| Fleet host via Google — completion form pattern | **Google handles auth; the role-specific completion page (`/signup/complete?role=host&kind=FLEET`) collects the FLEET-only fields (companyName, businessRegNumber, serviceArea) before creating the `Owner` row.** Independent host has the same page minus those fields. Customer has the page minus those fields too — only fullName is captured (matching email/password). Email comes from the authenticated Supabase session, never from the form | Tier 18 | Google can't capture business-registration data, so we collect it post-OAuth. The completion page mirrors the existing email/password signup form fields exactly so behavior + DB shape converge. Reading email from the session (not form input) prevents a user from claiming a different identity than they authenticated with. |
| Dev/prod environment separation | **Deferred — single Supabase project (`CarBNB`) shared between local dev and prod deployment for now.** Postgres is already separated (Docker locally, Supabase Postgres in prod). Auth and Storage stay shared. Logged as a pre-public-launch task: split into a second `carbnb-dev` Supabase project, audit + delete test users, scope Vercel preview/prod env vars separately | Pre-Tier-18 housekeeping | Free-tier limit of 2 Supabase projects per organization, and `internz.2026@gmail.com` already had 2 projects on it. Splitting is doable but blocked on freeing a slot or paying. The marketplace isn't public-launched yet, so the practical risk of shared auth users is low. Revisit before public launch. |
| Booking-creation gating strength on customer verification | **Block at booking creation.** Server action `createBookingAction` rejects with a clear error if `customer.status !== VERIFIED`. UI on `/listings/[id]` hides the Reserve button and shows a "Verify your identity to book" link card instead. Customers can still browse listings without verification | Tier 19 | User picked the stricter option over "PENDING booking allowed, CONFIRMED requires verified". Cleaner data — no dangling PENDING bookings from unverified accounts. Upfront friction is acceptable for a P2P car-rental marketplace where verification is expected (Turo / Airbnb do this). The booking gate is enforced both client + server so a stale tab / direct API hit can't bypass it. |
| Required documents for customer verification | **Government ID + Driver's License, both required.** Stored as nullable columns on Customer (`idDocumentUrl`, `licenseDocumentUrl`); admin can't flip status to VERIFIED until both are uploaded (enforced via UI states, not action gate — admin can technically force-verify someone with no docs by clicking the button, but the UX disincentivizes this) | Tier 19 | Gov ID confirms identity; driver's license confirms they can legally drive a car. Same friction shape as host onboarding. License alone is a weaker signal because it could be expired or fake; gov ID cross-references. |
| Selfie / liveness check | **No — host inspects ID at pickup as the live check.** No third-party identity service, no selfie upload field on Customer. Future tier if abuse appears | Tier 19 | V1 scope. The host-meets-customer-at-handover step provides the real-world live check. Adding selfie + liveness verification would require either a third-party API integration or significant in-house image comparison logic. Not justified for the demo / pre-launch marketplace size. |
| Re-verification cadence | **One-time verification + admin manual flag for re-verification.** No cron, no expiry date. Admin can click "Re-verify" on a VERIFIED customer's detail page to flip them back to PENDING (docs stay attached; customer re-uploads to replace) | Tier 19 | Avoids infra weight (scheduled jobs, email reminders) for a use case that's better handled case-by-case. Admin gets a manual lever for "this license has obviously expired" or "complaints came in, re-check." Yearly automatic re-verification can be added later if needed without touching the data model. |
| autoApproveVerifiedCustomers wiring | **When the platform setting is on, a VERIFIED customer's new booking is created with `status = CONFIRMED` instead of `PENDING`.** Verification still requires manual admin review — the setting only affects per-booking host approval, not the verification step itself. Setting defaults to OFF; admin toggles in `/settings` | Tier 19 | Field name's intent literally is "auto-approve VERIFIED customers", so the wiring should match. Verifying a customer is already a manual gatekeeping step; making them go through host approval every booking afterward is redundant friction. The toggle stays admin-controlled so a stricter policy is one click away. |
| Existing customers on migration | **Backfilled all existing Customer rows to `status = "Verified"`** in the same migration that added the column (UPDATE statement after the ADD COLUMN). New customers signup with PENDING explicitly set in `signupAction` and `completeProfileAction` | Tier 19 | Existing seeded + dummy customers shouldn't suddenly be unable to book. Pre-public-launch the customer table is mostly test data, so the data-integrity cost of grandfathering is low. Real launch will require deleting / re-verifying these accounts as part of the broader pre-launch cleanup task. |
| Fleet host driver's license requirement | **Skip the license entirely for FLEET kind hosts.** Required docs per kind: INDIVIDUAL = ID + license; FLEET = ID + business registration certificate. Per-kind matrix encoded in `lib/host-verification.ts` and surfaced through the upload UI on `/host/profile` + admin `/owners/[id]/edit` | Tier 19 | The contact person on a FLEET account manages cars; they don't personally drive them. Drivers a fleet employs are out-of-platform-scope (real fleet management software would track those separately). For V1, the fleet contact person's identity (gov ID) + business legitimacy (DTI / SEC cert) is sufficient. Adding driver coverage in the future is a separate model + tier. |
| Host doc-submission state derived (no explicit Submit button) | **A host appears in the admin verification queue once all required docs are uploaded for their kind — no separate "Submit for Verification" column or button.** Status banner on `/host/profile` reflects the implicit state ("Documents submitted — awaiting review.") | Tier 19 | Avoids a third Owner state (between "no docs" and "PENDING admin queue") and the schema column that would need it. The host UX still has a clear "you're in the queue" signal via the banner — they just don't have to click an extra Submit button after uploading. Cheaper than Tier 17's explicit DRAFT→PENDING_APPROVAL transition because Owner.status already has a meaningful PENDING value to filter on. |
| Customer-documents Supabase Storage bucket | **New private bucket `customer-documents` created in the shared Supabase project, mirroring the `owner-documents` bucket pattern.** Upload + signed URL access both via service-role key (no RLS policies required); 1-hour signed URL TTL | Tier 19 | Same shape as the existing `owner-documents` bucket so all the existing patterns transfer. Service-role key + signed URLs gives us write-once / read-with-expiring-link without any RLS complexity. Bucket creation is a one-time manual step in the Supabase dashboard, documented in MANUAL-TESTING.md Tier 19 prerequisites. |
| Email provider | **Resend.** Free tier 3,000 emails/mo + 100/day. React-email JSX templates co-located in `emails/`. SendGrid / Postmark considered but Resend wins on Next.js developer experience | Tier 20 | Built specifically for this stack. Single `resend` package install, one `RESEND_API_KEY` env var, one `lib/notify.ts` helper. Template lives as a JSX component, so changes ship through the same review process as the rest of the app. |
| Email From address | **Resend sandbox `DriveXP <onboarding@resend.dev>` for V1.** Custom-domain From (e.g. `noreply@hoversight.agency`) deferred to launch-polish — needs DNS records (SPF/DKIM) added in Hostinger, ~30 min one-time setup. Upgrade is a one-line change to `EMAIL_FROM` in `lib/notify.ts` | Tier 20 | Sandbox unblocks shipping without DNS work. Header reads slightly unprofessional but users click email content not the header. Bundle the custom-domain upgrade with Google brand verification + the `/privacy` /`terms` pages in launch polish. |
| Notification preferences (per-user opt-out) | **None in V1.** All users receive all notifications relevant to their role. No toggles, no per-type matrix, no schema columns for preferences | Tier 20 | Over-engineering for pre-launch. Tier 20 events are all genuinely actionable (booking activity, account state changes) — not the kind of "marketing email" people want to suppress. If real users complain about volume, V2 can add a simple master toggle without breaking changes. |
| Trip-start reminder notifications | **Parked for a small follow-up tier.** Daily cron job to scan upcoming bookings + fire "your rental starts tomorrow" notifications is a different infrastructure concern (Vercel Cron) than the event-driven notify() shape | Tier 20 | Mixing cron with event-driven notifications muddles two mental models. Trip reminders are valuable but bounded — better as a focused follow-up tier where the cron setup gets full attention. |
| notify() helper architecture | **Single `lib/notify.ts` does both DB row + email send per call.** In-app notification (Notification table) is the source of truth — always written. Email is best-effort via Resend; failures logged but don't bubble. `notifyMany()` for fan-out (BOOKING_CREATED → host + admin). Audit logged to ActivityLogEntry with success/skip detail | Tier 20 | One helper means each action's diff is one `await notify({...})` line. Email failures must not block actions — in-app bell is the reliable channel; email is the convenience layer. Future SMS / push channels slot inside the same helper. |
| Notification trigger points (V1) | **11 events** wired in Tier 20: BOOKING_CREATED (host + admin), BOOKING_CONFIRMED (customer), BOOKING_REJECTED (customer), BOOKING_CANCELLED (host), OWNER_VERIFIED (host), OWNER_SUSPENDED (host), LISTING_APPROVED (host), LISTING_REJECTED (host), CUSTOMER_VERIFIED (customer), CUSTOMER_REJECTED (customer), REVIEW_POSTED (host). Booking lifecycle progressions (start / mark-paid / complete) deliberately NOT notified — they're informational and don't change required actions | Tier 20 | The 11 events are where the recipient needs to know AND can act (or wants the outcome). Adding more (e.g. host starts a rental) creates noise without value. Easier to add later than to silence loud notifications. |
| In-app refresh strategy | **No polling in V1 — bell re-renders on every Next.js page navigation.** `<NotificationBellServer>` server component fetches last 5 + unread count per render. Server actions revalidate paths after the mutation, so notifications appear on the next render. Real-time polling (matching Tier 14 chat 5s cadence) deferred until usage shows navigation-driven refresh is insufficient | Tier 20 | Polling adds load with marginal UX gain for low-traffic. Bell badge updates on next page click — acceptable for V1. Add `useInterval` polling later as one-line change in the bell client component if needed. |
