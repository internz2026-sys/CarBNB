...
export function ListingPhotoGallery({
  ...
  const [uploadState, uploadAction, uploadPending] = useActionState<
    ListingActionState,
    FormData
  >(addListingPhotoAction, null);
  const [removeState, removeAction] = useActionState<ListingActionState, FormData>(
    removeListingPhotoAction,
    null,
  );
  const [primaryState, primaryAction] = useActionState<ListingActionState, FormData>(
    setPrimaryPhotoAction,
    null,
  );
  const [moveState, moveAction] = useActionState<ListingActionState, FormData>(
    movePhotoAction,
    null,
  );
      ...
      <CardContent className="space-y-6">
        ...
        {photos.length === 0 ? (
          <div className="rounded-lg bg-muted/30 p-8 text-center text-sm text-muted-foreground">
            No photos uploaded yet. Add at least one before approving the listing.
          </div>
        ) : (
